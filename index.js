...
const formatEnforcer = {
    enforceWordLimit(reasoning, maxWordsPerStep) {
        if (!maxWordsPerStep)
            return reasoning;
        // Split into steps
    ...
    analyzeAdherence(reasoning, maxWordsPerStep) {
        // Split into steps
        const stepPattern = /(\d+\.\s*|Step\s+\d+:|\n-\s+|\n\*\s+|•\s+|^\s*-\s+|^\s*\*\s+)/;
        let steps = reasoning.split(stepPattern).filter(Boolean);
        let totalWords = 0;
...
const reasoningSelector = {
    ...
    selectApproach(domain, complexity, performanceStats) {
...
const chainOfDraftClient = {
    async solveWithReasoning(params) {
        const { problem, domain = 'general', max_words_per_step = null, approach = null, enforce_format = true, adaptive_word_limit = true } = params;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "chain_of_draft_solve" && args?.problem) {
            ...
            const formattedResponse = `Chain of ${result.approach} reasoning (${result.word_limit} word limit):\n\n` +
